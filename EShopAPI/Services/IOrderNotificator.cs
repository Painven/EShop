...
public interface IOrderEmailNotificator
{
    Task SendNewOrderNotification(string customerEmail, int orderNumber);
}

public class MailKitOrderEmailNotificator : IOrderEmailNotificator
{
    private readonly IConfiguration configuration;
    private readonly ILogger logger;

    public MailKitOrderEmailNotificator(IConfiguration configuration, ILogger logger)
    {
    ...
    public async Task SendNewOrderNotification(string customerEmail, int orderNumber)
    {
        ...
        try
        {
            ...
            smtp.Connect(serverSmtpHost, serverSmtpPort, SecureSocketOptions.StartTls);
        ...
        {
            logger.LogError($"Ошибка отправки Email '{serverUsername ?? string.Empty}' --> '{customerEmail ?? String.Empty}'");
        }
    }
}
